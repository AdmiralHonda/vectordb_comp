...
# データベースに接続
connections.connect(alias=connection_alias, host="standalone", port="19530")

# コレクションの各要素を定義
content_id = FieldSchema(
    name="id",
...
content_title = FieldSchema(
    name="title",
    dtype=DataType.VARCHAR,
    max_length=200,
)

content_description = FieldSchema(
    name="description",
    dtype=DataType.VARCHAR,
    max_length=200,
)
...
# 定義した各要素をまとめてコレクションを作成
content_schema = CollectionSchema(
    fields=[content_id,content_title,content_description,content_vector],
    description="Test Amazon Content Search",
)
collection_name = "TestContentSearch"
...
insert_data = [
    ...
    [encoder.encode(doc["description"]) for doc in documents]
]

# データの挿入
err = content_collection.insert(insert_data)
...
index_params = {
  "metric_type":"COSINE",
  "index_type":"HNSW",
  "params": {"M": 30, "efConstruction": 100}
}
...
# 検索
# 1. 検索できるようにメモリにデータをロードする
content_collection = Collection(collection_name)
...
search_results = content_collection.search(
    data=[encoder.encode("alien invasion").tolist()],
    anns_field="vector",                                # 検索対象のフィールド
    limit=3,
    param=content_search_params,
    output_fields=["title"]
)

for response in search_results[0]:
    print(response.entity.get("title"))